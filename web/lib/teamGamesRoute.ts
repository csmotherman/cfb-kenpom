...
// Shared handler for the per-team, per-game "custom sample" endpoints
// (Advanced and Exploratory). The season artifact lives in premium_datasets as
// week=1 of its dataset type -- a multi-megabyte JSON -- so it is NEVER pulled
// whole: PostgREST extracts `meta` and the one requested team inside Postgres and
// only that slice (~5-13 KB) crosses the wire. If the path select is ever
// rejected, a short-lived in-memory cache keeps the fallback (a full read) to at
// most one per instance per TTL instead of one per click.
type Kind = "advanced" | "exploratory";
type Slice = { meta: unknown; team: unknown };
...
const fallbackCache = new Map<string, { at: number; payload: { meta: unknown; teams: Record<string, unknown> } }>();

async function readSlice(kind: Kind, season: number, teamId: string): Promise<Slice | null> {
  const admin = createAdminClient();
  const narrow = await admin
    .from("premium_datasets")
    .select(`meta:payload->meta,team:payload->teams->t${teamId}`)
    .eq("dataset_type", kind)
    .eq("season", season)
    .eq("week", 1)
    .maybeSingle();
  ...
  const full = await admin.from("premium_datasets").select("payload").eq("dataset_type", kind).eq("season", season).eq("week", 1).maybeSingle();
  if (full.error) throw full.error;
  if (!full.data) return null;
  const payload = full.data.payload as { meta: unknown; teams: Record<string, unknown> };
  fallbackCache.set(cacheKey, { at: Date.now(), payload });
  return { meta: payload.meta, team: payload.teams[`t${teamId}`] ?? null };
}

export async function teamGamesResponse(kind: Kind, year: string, teamId: string) {
  ...
  try {
    const slice = await readSlice(kind, Number(year), teamId);
