...
from cfb_analytics.analytics.dropback_v1_candidate import audit_candidate,merge,concise
...
def main():
 ...
 for s in ((a.season,) if a.season is not None else SEASONS):
  for st,w in discover_partitions(a.root,s):
   plays=json.loads((canonical_partition_dir(a.processed_root,s,st,w)/"plays.json").read_text());drives=json.loads((derived_drive_partition_dir(a.processed_root,s,st,w)/"drives.json").read_text());rs.append(audit_candidate(plays,drives))
 r=merge(rs);print(json.dumps({"counts":r["counts"]},indent=2,sort_keys=True) if a.as_json else concise(r))
