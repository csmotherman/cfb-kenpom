...
from cfb_analytics.analytics.state_transition_pass_forensics import audit,merge,concise
...
def main():
 ...
 for s in ((a.season,) if a.season is not None else SEASONS):
  for st,w in discover_partitions(a.root,s):
   plays=json.loads((canonical_partition_dir(a.processed_root,s,st,w)/"plays.json").read_text());rs.append(audit(plays))
