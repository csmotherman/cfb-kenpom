...
from cfb_analytics.analytics.first_down_generation_candidate_forensics import audit_partition,merge,concise
...
def main():
 ...
 for s in ((a.season,) if a.season else SEASONS):
  for st,w in discover_partitions(a.root,s):
   plays=json.loads((canonical_partition_dir(a.processed_root,s,st,w)/"plays.json").read_text());drives=json.loads((derived_drive_partition_dir(a.processed_root,s,st,w)/"drives.json").read_text());rs.append(audit_partition(drives,plays))
