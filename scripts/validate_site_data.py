...
def validate_season(rankings, advanced, *, previous=None):
    ...
    return {"weeks": len(weeks), "teams": len(last), "rated": sum(r["rank"] is not None for r in last.values()), "games": int(games / 2), "teamGamesWithoutPlayStats": int(games - games_with_stats)}
...
def main():
    ...
    for year in meta["rankingsYears"]:
        ...
        a = json.loads((args.data_dir / "advanced" / f"{year}.json").read_text())
