...
- Expanded Model Performance: **partly done**. Live record split by model, RMSE, and a labelled historical backtest with out-of-sample calibration are published (`scripts/build_model_backtest.py`). Market MAE, closing-favorite accuracy and model-market disagreement wait on closing-line data (next item).
