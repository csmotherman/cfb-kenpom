...
- PRIME vs market benchmark: **done** (`scripts/build_market_benchmark.py`, `docs/market_benchmark.md`). Result: the closing line beats PRIME on winner accuracy, margin error and log loss; PRIME-vs-line disagreements carry no reliable signal.
- Expanded Model Performance: **done**, including live and backtest market comparison. Earlier note:  Live record split by model, RMSE, and a labelled historical backtest with out-of-sample calibration are published (`scripts/build_model_backtest.py`). Market MAE, closing-favorite accuracy and model-market disagreement wait on closing-line data (next item).
